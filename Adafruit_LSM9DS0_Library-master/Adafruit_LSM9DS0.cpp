...
bool Adafruit_LSM9DS0::begin()
{
  if (_i2c) {
    ...
    pinMode(_csg, OUTPUT);
    digitalWrite(_csxm, LOW);
    digitalWrite(_csg, LOW);
    SPI.begin();
  } else {
    // Sofware SPI
    pinMode(_clk, OUTPUT);
...
void Adafruit_LSM9DS0::calcVelocity(sensors_event_t* prevAccel, sensors_event_t* accel, float timeDif) {
  ...
  velocity.z += (((prevAccel->acceleration.z + accel->acceleration.z)/2)*timeDif)/1000000;
  velocity.magnitude = sqrt(sq(velocity.x)+sq(velocity.y)+sq(velocity.z));
}

void Adafruit_LSM9DS0::calcDistance(float timeDif) {
  // Calculate the distance by integrating velocity (trapezoidal method)
  distance.x += (((prevVelocity.x + velocity.x)/2)*timeDif)/1000000;
  distance.y += (((prevVelocity.y + velocity.y)/2)*timeDif)/1000000;
  distance.z += (((prevVelocity.z + velocity.z)/2)*timeDif)/1000000;
  distance.magnitude = sqrt(sq(distance.x)+sq(distance.y)+sq(distance.z));
}
